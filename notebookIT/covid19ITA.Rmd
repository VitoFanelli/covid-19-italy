...
covid19 <- covid19 %>% 
  ...
  mutate(
    ...
    variazPerc_tasso_contagi_x_1000 = c(NA, 
      round(diff(tasso_contagi_x_1000)/tasso_contagi_x_1000[-n()]*100,2)),
    variazPerc_tasso_mortalita_x_100 = c(NA, 
      round(diff(tasso_mortalita_x_100)/tasso_mortalita_x_100[-n()]*100,2))
  ) %>% 
  ungroup()
...
**`r paste0("POSITIVI al ", max(italia$data), ": ", formatC(italia$contagi[italia$data == max(italia$data)] - italia$dimessi[italia$data == max(italia$data)] - italia$decessi[italia$data == max(italia$data)], big.mark = "."))`**
...
highchart(type = "stock") %>% 
  hc_add_series(covid19, hcaes(x = data, y = variazPerc_tasso_contagi_x_1000, 
    group = regione), type = "line") %>% 
  hc_legend(enabled = T) %>% 
  hc_title(text = "Variazione % Tasso di contagio (x 1000 abitanti)") %>%
  hc_xAxis(title = list(text = "Giorno")) %>% 
  hc_yAxis(title = list(text = "Variazione %"))
```
...
highchart(type = "stock") %>% 
  hc_add_series(covid19, hcaes(x = data, y = variazPerc_tasso_mortalita_x_100, 
    group = regione), type = "line") %>% 
  hc_legend(enabled = T) %>% 
  hc_title(text = "Variazione % Tasso di mortalità (x 100 contagi)") %>%
  hc_xAxis(title = list(text = "Giorno")) %>% 
  hc_yAxis(title = list(text = "Variazione %"))
```
