...
highchart(type = "stock") %>% 
  ...
  hc_add_series(italia, hcaes(x = data, y = tasso_mortalita_x_100,2),
    type = "line", name = "Tasso di mortalità x 100 infetti", yAxis = 1) %>% 
  hc_legend(enabled = T) %>% 
  hc_title(text = paste0("Situazione Italia al ", max(italia$data))) %>%
...
highchart(type = "stock") %>% 
  ...
  hc_add_series(prediction20, hcaes(x = data, y = floor(pred)),
                ...
                color = "#b3e6b3") %>% 
  hc_legend(enabled = T) %>% 
  hc_title(text = paste0("Previsioni Contagi al ", min(prediction5$data)-1)) %>% 
...
datatable(tabella, caption = "I dati sono cumulati a partire dal 24/02/2020")
```
...
highchart(type = "stock") %>% 
  ...
  hc_legend(enabled = T) %>% 
  hc_title(text = "Tasso di contagio (x 1000 abitanti) per Giorno/Regione") %>%
...
highchart(type = "stock") %>% 
  hc_add_series(covid19, hcaes(x = data, y = tasso_mortalita_x_100, 
    group = regione), type = "line") %>% 
  hc_legend(enabled = T) %>% 
  hc_title(text = "Tasso di mortalità (x 100 infetti) per Giorno/Regione") %>%
  hc_xAxis(title = list(text = "Giorno")) %>% 
...
highchart(type = "stock") %>% 
  hc_add_series(covid19, hcaes(x = data, y = dimessi_decessi, 
    group = regione), type = "line") %>% 
  hc_legend(enabled = T) %>% 
  hc_title(text = "Rapporto Dimessi-Decessi per Giorno/Regione") %>%
  hc_xAxis(title = list(text = "Giorno")) %>% 
